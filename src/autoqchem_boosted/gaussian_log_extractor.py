from autoqchem_boosted.helper_functions import *
import json

logger = logging.getLogger(__name__)
float_or_int_regex = "[-+]?[0-9]*\.[0-9]+|[0-9]+"

def retrieve_unfinished_gaussianjobs(molecule_ids):
    errored_molecules = []

    for molecule_id in molecule_ids:
        log_file_path = f'{directory}/gaussian/outputfiles/{molecule_id}/conf_0.log'
        with open(log_file_path) as f:
            log = f.read()
        normal_termination_flag = len(re.findall("Normal termination", log))
        if normal_termination_flag == 0: errored_molecules.append(molecule_id)
        
    return errored_molecules
...
class gaussian_log_extractor(object):
    ...
    def __init__(self, log_file_path):
        ...
        smiles_molid_df = pd.read_csv(f'{main_datasets_dir}/retrospective/suzuki_dataset_unique_molecules_fragments_removed.csv')
        self.molid = log_file_path.split('/')[-2]
        self.smiles = smiles_molid_df.loc[smiles_molid_df['Molecule ID'] == self.molid, 'SMILES'].iloc[0]
    ...
    def get_descriptors(self) -> dict:
        ...
        keys_to_save = ['smiles', 'labels', 'atommaps', 'descriptors', 'atom_descriptors', 'transitions', 'modes', 'mode_vectors', ]
    ...
    def _extract_descriptors(self) -> None:
        ...
        self.get_atom_labels()  # atom labels and mapping
        #pdb.set_trace()
    ...
    def get_atom_labels(self) -> None:
        ...
        for line in z_matrix:
            ...
            if len(space_split) > 1:
                ...
                if space_split[0] != 'H': self.atommaps.append(count)
                count += 1
...
# converts log files to json files for molecules given by list of molecule ids
def convert_logfiles_to_jsonfiles(molecule_ids):
    smiles_id_df = pd.read_csv(f'{main_datasets_dir}/retrospective/suzuki_dataset_unique_molecules_fragments_removed.csv')
    smiles_list, molecule_id_list = [], []

    for i,molecule_id in enumerate(molecule_ids):
        smi = smiles_id_df.loc[smiles_id_df['Molecule ID'] == molecule_id, 'SMILES'].item()
        file_path = f"{directory}/outputfiles/{molecule_id}/conf_0.log"
        gle = gaussian_log_extractor(file_path)
        if gle.n_tasks > 0: # if the gaussian job was successful
            smiles_list.append(smi)
            molecule_id_list.append(molecule_id)
            desc = json.dumps(gle.get_descriptors())
            with open(f"{main_datasets_dir}/molecule_descriptors/{molecule_id}.json", "w") as outfile:
                outfile.write(desc)
    return
